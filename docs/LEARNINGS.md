...
### Mirroring backend DTOs as TypeScript interfaces
- `products.ts` defines `Topic`, `ProductScreenshot`, `ProductSummary`, `ProductDetail` as TypeScript interfaces shaped exactly like the backend's `TopicDto`/`ProductScreenshotDto`/`ProductSummaryDto`/`ProductDetailDto` records — same relationship as the backend DTOs have to the database entities: a stable, explicit contract for data crossing a boundary (here, network → frontend, instead of database → backend).
- `interface ProductDetail extends ProductSummary` mirrors the actual API shape (the detail response is a strict superset of the summary response, adding `description`, `websiteUrl`, `screenshots`) — TypeScript's `extends` lets the type system enforce that relationship instead of duplicating the shared fields.

### `fetch()` doesn't throw on HTTP error statuses
- A common gotcha: `fetch()`'s promise only rejects on a genuine network failure (DNS failure, connection refused, offline) — a 404 or 500 response is still a "successful" fetch as far as the Promise is concerned; `res.ok` is `false`, but nothing throws automatically.
- **Why this matters in practice:** without an explicit `if (!res.ok) throw ...` check, calling `.json()` on a 404 response would either throw a confusing parse error (if the error body isn't JSON) or silently succeed with whatever error payload the server sent, treated as if it were real product data.
- `products.ts`'s `fetchProducts`/`fetchProduct` both check `res.ok` explicitly and throw a clear `Error` before ever touching `.json()`, so failures surface predictably.

### Component split: presentational vs. container, a real instance of a common React pattern
- `ProductCard` is purely presentational — takes a `product` prop, renders it, owns no state, makes no network calls. `ProductList` is the "container" — owns `products`/`loading`/`error` state via `useState`, does the `fetchProducts()` call in `useEffect`, and maps results into `ProductCard`s.
- **Why split this way instead of one component doing everything:** `ProductCard` becomes trivially reusable (a future detail page could reuse it, or use `ProductDetail` for extra fields) and easy to reason about in isolation — its output is a pure function of its props. `ProductList` is where all the "what if the fetch fails / is still loading" complexity lives, in exactly one place.
- **`.finally(() => setLoading(false))`** ensures loading state clears on both the success and failure paths — a common bug is only clearing it inside `.then()`, leaving the UI stuck on "Loading..." forever if the fetch throws.

### `.claude/launch.json` — configuring a project's dev server for the built-in browser preview
- Claude Code's browser-preview tool (`preview_start`) needs a small project-local config file describing how to start a dev server: which command (`npm`), which arguments (`run dev`, plus `--prefix source/frontend` since the frontend lives in a subfolder, not the repo root), and which port to open (`5173`).
- This is a one-time setup file, not app code — it lives in `.claude/` alongside the two skills, and lets the frontend be verified visually (screenshot, page text, console errors) inside the session itself instead of only trusting that "the code looks right."
- **Verified this way:** the full Phase 1 stack (Postgres → Spring Boot → React) rendering 9 real launched products, zero console errors — the actual walking-skeleton verification, not just a code read-through.

### Evaluating a third-party skill before installing it — and deciding not to, based on actual project goals
- A community Claude Code skill (`ui-ux-pro-max`) was looked into as a way to speed up frontend styling — a local Python (stdlib-only) tool that matches a UI/UX request against a bundled catalog of styles/colors/fonts/rules and returns a design system plus an accessibility checklist.
- **A concrete red flag worth remembering as a general lesson:** the repo claimed a very high star count (~129k) for a repository created under 4 months earlier, and the GitHub stargazers page didn't actually load when checked — a mismatch between a claimed popularity signal and what could be independently verified. Doesn't prove the skill is unsafe, but it's a reason not to trust "looks popular" at face value before installing something that runs local scripts against your project.
- **The actual reason it was declined, though, was simpler than the trust question:** re-checking the project's real goal (learning backend systems deeply — internals, communication patterns, etc. — with frontend fluency being a deliberately later, sequenced goal) showed the skill solves a problem (frontend design decisions) that isn't the current priority at all. A tool can be technically fine and still be the wrong thing to add right now if it's optimizing for a goal you're not actively pursuing.
- **General takeaway:** before adopting a new tool/skill, it's worth checking two independent things separately — "is this trustworthy" and "does this actually serve what I'm trying to learn/build right now" — a tool can pass one check and fail the other.
